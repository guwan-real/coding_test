...
LOG_DIR="${LOG_DIR:-${RUNS_DIR}/train_line_ffn_focal/${STAMP}}"
mkdir -p "${LOG_DIR}"

COMPRESSION_HEAD_TYPE="${COMPRESSION_HEAD_TYPE:-ffn}"
COMPRESSION_LOSS_TYPE="${COMPRESSION_LOSS_TYPE:-focal}"
FOCAL_ALPHA="${FOCAL_ALPHA:-0.65}"
FOCAL_GAMMA="${FOCAL_GAMMA:-2.0}"
AUTO_FOCAL_ALPHA="${AUTO_FOCAL_ALPHA:-0}"

EXTRA_ARGS=()
if [[ "${AUTO_FOCAL_ALPHA}" == "1" ]]; then
  EXTRA_ARGS+=(--auto-focal-alpha)
else
  EXTRA_ARGS+=(--focal-alpha "${FOCAL_ALPHA}")
fi
...
bash "${PROJECT_ROOT}/train/train_llm.sh" "${NUM_GPUS}" "${TRAIN_JSONL}" \
  ...
  --compression-head-type "${COMPRESSION_HEAD_TYPE}" \
...
echo "Training run finished: ${LOG_DIR}"
