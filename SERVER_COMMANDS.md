...
## 4. Train the current line-label FFN/focal candidate
...
```bash
CUDA_VISIBLE_DEVICES=0,1 NUM_GPUS=2 BATCH_SIZE=8 EPOCHS=3 bash scripts/train_line_ffn_focal.sh
COMPRESSION_HEAD_TYPE=crf AUTO_FOCAL_ALPHA=1 bash scripts/train_line_ffn_focal.sh
```

Outputs go to `runs/train_line_ffn_focal/<timestamp>/`.
